...
popularRate.enter(async (ctx) => {
  ...
  await ctx.replyWithMarkdown(
    "🎶 *Оценить треки* \n\n*Новый алгоритм!* Бот присылает тебе по порядку все треки начиная с промо раунда ПВБ9. С этого момента репер не сможет оценить свои треки, а все остальные *не будут повторяться*. Таким образом каждый трек с батла получится оценить *только один раз*! Кнопка 💖 это +1 балл, а кнопка 💩 это -1 балл."
  );
  await ctx.replyWithMarkdown(
